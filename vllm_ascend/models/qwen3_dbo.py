...
class Qwen3MoeDecoderLayerDBO(Qwen3MoeDecoderLayer):
    ...
    def _forward_ms_layer_alltoallv_finegrained(
            ...
            chunked_hidden_states_sizes=[None, None]
    ):
        ...
        for i in range(num_micro_batchs):
            ...
            #     print(ms_metadata.ms_events[layer_index-1][i][MSEventKey.FFN_AR_FINISH])
        ...
        for i in range(num_micro_batchs):
            ...
            with torch.npu.stream(combine_context.comm_stream):
                ...
                ms_metadata.ms_events[layer_index][i][
                    MSEventKey.FFN_AR_FINISH] = combine_context.comm_stream.record_event()


        return hidden_states, residual, num_tokens, chunked_hidden_states_sizes
...
class CustomQwen3DBOMoEModel(Qwen3MoeModel):
    ...
    def can_run_ms(self):
        ...
        if attn_metadata is None or not with_prefill or not attn_metadata.enable_dbo_across_dp:
            # if attn_metadata is None or not attn_metadata.enable_dbo_across_dp:
            # if torch.distributed.get_rank() == 0:
            #     print(with_prefill, getattr(attn_metadata, 'num_actual_tokens', None))
            return False
        # if torch.distributed.get_rank() == 0:
        #     print(attn_metadata)
    ...
    def _forward_ms_layers(
        ...
        # exit()
        # the rest layers
        num_tokens, chunked_hidden_states_sizes = [None, None], [None, None]
        for i in range(moe_start_layer, self.end_layer):
            layer = self.layers[i]
            ms_layer_forward_func = layer._forward_ms_layer_alltoallv_finegrained
            hidden_states, residual, num_tokens, chunked_hidden_states_sizes = ms_layer_forward_func(
                positions=positions,
                hidden_states=hidden_states,
                residual=residual,
                attn_metadata=attn_metadata,
                num_tokens=num_tokens,
                chunked_hidden_states_sizes=chunked_hidden_states_sizes
            )
        ...
        for i in range(2):
            ms_metadata.try_wait_event(layer_index - 1, i, MSEventKey.FFN_AR_FINISH)
            if getattr(ms_metadata, "last_layer_dispatchers", None) is not None:
                hidden_states[i] = ms_metadata.last_layer_dispatchers[i].unpermute2()
                if self.tp_size > 1:
                    hidden_states[i] = gather_from_sequence_parallel_region(hidden_states[i], self.tp_group,
                                                                            chunked_hidden_states_sizes[i])
                    if num_tokens[i] < self.tp_size:
                        hidden_states[i] = hidden_states[i][:num_tokens[i]]

        ms_metadata.last_layer_dispatchers = None

        # ms_metadata.try_wait_event(layer_index - 1, 0, MSEventKey.FFN_AR_FINISH)
        # ms_metadata.try_wait_event(layer_index - 1, 1, MSEventKey.FFN_AR_FINISH)
